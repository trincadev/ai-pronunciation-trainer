...
* generation: <https://www.neant.ro/uncategorized/generate-self-signed-tls-certificates-ecdsa-edition.html>
* verification: <https://www.neant.ro/uncategorized/generate-self-signed-tls-certificates-ecdsa-edition.html>

```
CERTFILEPREFIX=https_localhost
openssl ecparam -name secp384r1 -genkey -noout -out $CERTFILEPREFIX.key
openssl req -new -x509 -key $CERTFILEPREFIX.key -out $CERTFILEPREFIX.crt -days 1095 -config $CERTFILEPREFIX.cnf

openssl ec -in $CERTFILEPREFIX.key -pubout | openssl sha256
penssl x509 -in $CERTFILEPREFIX.crt -pubkey -noout | openssl sha256
```

### Content of https_localhost.cnf
